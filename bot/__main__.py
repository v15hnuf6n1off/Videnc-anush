...
async def _(event):
        if str(event.sender_id) not in OWNER and event.sender_id !=DEV:
            return await event.reply_text("**Sorry❗️ You're not An Authorised User!**")
        if not event.photo:
            return
        os.system("rm thumb.jpg")
        await event.client.download_media(event.media, file="/bot/thumb.jpg")
        await event.reply("**Thumbnail 🖼 Saved Successfully✅️.**")
            ...
            if not WORKING and QUEUE:
                ...
                with open(out, "rb") as f:
                    ok = await upload_file(
                        ...
                        name=out,
                        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                            progress(d, t, nnn, ttt, f"** Uploading 📤 File**\n__{out.replace(f'encode/', '')}__")
                        ),
                    )
                await nnn.delete()
